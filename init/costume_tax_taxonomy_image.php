...
add_action( 'costume_tax_add_form_fields', 'add_costume_rent_custom_taxonomy_image', 10, 2 );
function add_costume_rent_custom_taxonomy_image ( $taxonomy ) {
?>
...
add_action( 'created_costume_tax', 'save_costume_rent_custom_taxonomy_image', 10, 2 );
function save_costume_rent_custom_taxonomy_image ( $term_id, $tt_id ) {
...
add_action( 'costume_tax_edit_form_fields', 'update_costume_rent_custom_taxonomy_image', 10, 2 );
function update_costume_rent_custom_taxonomy_image ( $term, $taxonomy ) { ?>
...
add_action( 'edited_costume_tax', 'updated_costume_rent_custom_taxonomy_image', 10, 2 );
function updated_costume_rent_custom_taxonomy_image ( $term_id, $tt_id ) {
    if( isset( $_POST['image_id'] ) && '' !== $_POST['image_id'] ){
        $image = $_POST['image_id'];
...
add_action( 'admin_enqueue_scripts', 'costume_rent_custom_taxonomy_load_media' );
function costume_rent_custom_taxonomy_load_media() {
    if( ! isset( $_GET['taxonomy'] ) || $_GET['taxonomy'] != 'costume_tax' ) {
       return;
    }
    wp_enqueue_media();
}
...
//Custom script
add_action( 'admin_footer', 'add_costume_rent_custom_taxonomy_script' );
function add_costume_rent_custom_taxonomy_script() {
    if( ! isset( $_GET['taxonomy'] ) || $_GET['taxonomy'] != 'costume_tax' ) {
       return;
    }
    ?>
...
add_filter( 'manage_edit-costume_tax_columns', 'display_costume_rent_custom_taxonomy_image_column_heading' ); 
function display_costume_rent_custom_taxonomy_image_column_heading( $columns ) {
    $columns['category_image'] = __( 'Картинка', 'taxt-domain' );
    return $columns;
}

//Display new columns values
add_action( 'manage_costume_tax_custom_column', 'display_costume_rent_custom_taxonomy_image_column_value' , 10, 3); 
function display_costume_rent_custom_taxonomy_image_column_value( $columns, $column, $id ) {
